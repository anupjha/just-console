...
class Database {
  ...
  onInit() {
    ...
    this.db = new Localbase("just-console");
  ...
  async getCurrentUser() {
    try {
      if (localStorage.getItem("uid")) {
        ...
        let result = await this.db.collection("users").doc({ uid: uid }).get();
        return result;
      } else {
        return undefined;
      }
  ...
  async createUserSession(name) {
    try {
      let result = await this.db.collection("users").doc({ name: name }).get();
