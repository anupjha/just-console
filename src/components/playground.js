...
class Playground {
  ...
  appInit() {
    ...
    document.getElementById("html-code").innerHTML =
      this.currentProject.html.trim();
    document.getElementById("js-code").innerHTML =
      this.currentProject.js.trim();
    document.getElementById("css-code").innerHTML =
      this.currentProject.css.trim();

    this.htmlConfig = CodeMirror.fromTextArea(
    ...
    this.jsConfig = CodeMirror.fromTextArea(
    ...
    this.cssConfig = CodeMirror.fromTextArea(
  ...
  buildTabs() {
    this.tabsContent.forEach((item, i) => {
      if (this.tabs[i].getAttribute("name") === "js") {
        this.tabs[i].className = "tablink active d-block";
        item.className = "tabcontent d-block";
      } else if (this.tabs[i].getAttribute("name") === "css") {
        item.className = "tabcontent d-none";
        this.tabs[i].classList.remove("active");
      } else if (this.tabs[i].getAttribute("name") === "html") {
        item.className = "tabcontent d-none";
        this.tabs[i].classList.remove("active");
      } else {
  ...
  async updatePreview() {
    ...
    const preview =
      document.querySelector("#iframe-preview").contentWindow.document;
    preview.open();
  ...
  toggleSideBar() {
    document
      .querySelector("[data-sidenav-container]")
  ...
  async removeSession() {
    let res = confirm(
      "Are you sure that you want to remove the current session?"
    );
    if (res) {
  ...
  async saveProject(name) {
    if (!localStorage.getItem("uid")) {
      this.showMsg(
        "Project Not Saved",
        "You should connect to a session to be able to save this project",
        "danger"
      );
    } else {
      const state = this.currentProject.id ? "edit" : "new";
      const res = await this.db.saveProject(
        name,
        this.currentProject,
        this.images
      );
  ...
  async imageStream(file) {
    ...
    reader.onload = () => {
      ...
      this.images = [
        ...new Set([...this.images, { name: file.name, data: dataURL }]),
      ];
    };
